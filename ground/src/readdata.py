...
def read_compactfiles(indatafiles, inhkfiles, inspfiles, info, instrumentname, withdar = False, write=False ):
    ...
    sl = indata.noise_threshold.values #sensitivity limit given by radar software, in dB
    ...
    chirpstartidx = np.array(np.unique(indata.chirpseq_startix.values, axis=0)[0],dtype=int) #index in concatenated range array where chirp sequence starts
...
def read_kabandzenith(indatafiles, info, instrumentname, withdar = False, write=False):
    '''
    reads input Ka-band data.
    '''
